...
// ── Health (Already at /api/health due to file placement) ──────────
...
router.post('/api/users', async (request: IRequest, env: Env) => {
    try {
        ...
        await env.DB.prepare(
            ...
            id, body.tenant_id ?? null, body.full_name, body.email, body.role ?? 'user',
            body.status ?? 'active', body.company_name ?? '', now, now
        ).run();
        const profile = await env.DB.prepare('SELECT * FROM profiles WHERE id = ?').bind(id).first();
